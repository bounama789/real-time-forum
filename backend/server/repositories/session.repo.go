...
func (s *SessionRepository) UpdateSession(sess models.Session) error {
	err := s.DB.Update(s.TableName, sess, q.WhereOption{"sess_id": sess.SessId})
	if err != nil {
		return err
	}
	return nil
...
func (s *SessionRepository) DeleteSession(sessId string) error {
	err := s.DB.Delete(s.TableName, q.WhereOption{"sess_id": sessId})
	if err != nil {
		return err
	}
	return nil
...
func (s *SessionRepository) GetSession(sessId string) (models.Session, error) {
	...
	row,err := s.DB.GetOneFrom(s.TableName, q.WhereOption{"sess_id": sessId})
	if err != nil {
		return sess, err
	}
...
func (s *SessionRepository) GetSessionsByUserId(userId string) (sessions []models.Session, err error) {
	var sess models.Session
	rows,err := s.DB.GetAllFrom(s.TableName, q.WhereOption{"user_id": userId},"created_at DESC")
	if err != nil {
		return nil, err
	}
	for rows.Next(){
